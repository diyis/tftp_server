...
void initialize_server( tftp_t * instance ) {
    ...
    if ( instance->file )
        ...
        _err_log_exit( LOG_ERR, "Error from initialize_server() in strtok(file): File name not specified");
...
void data_send( tftp_t * instance ) {
    ...
    /* Verificamos que haya llegado un msg válido, se debe cumplir:
       ...
       5. Que el msg sea de donde lo esperamos (mismo tid del inicio de la transferencia) */
    ...
    syslog( LOG_NOTICE, "Retry number %d in data_send(); blknum %d", instance->retries,instance->blknum);
}
...
void start_data_send( tftp_t * instance ) {
    ...
    if ( access( instance->file, F_OK ) != 0 ) {
        ...
        syslog( LOG_ERR, "Error access() from start_data_send(): %s", strerror(errno));
...
void ack_send( tftp_t * instance ) {
    ...
    if ( received != -1
        ...
        /* Llegando un msg válido, reiniciamos a cero el número máximo de reintentos permitidos */
        ...
        if ( received < MAX_BUFSIZE ) {
            ...
            syslog(LOG_NOTICE, "File %s received successfully", instance->file );
    ...
    syslog(LOG_NOTICE, "Retry number %d in ack_send(); blknum %d",instance->retries,instance->blknum+1);
}
...
void start_ack_send( tftp_t * instance ) {
   ...
   syslog( LOG_NOTICE, "Receiving file %s", instance->file );
    ...
    syslog( LOG_NOTICE, "Receiving file %s ddddd", instance->file );
...
void child ( tftp_tl * listen ) {
    ...
    if ( setsockopt( instance->local_descriptor, SOL_SOCKET, SO_RCVTIMEO, (char *)&instance->timeout, sizeof( instance->timeout )) < 0 )
        _err_log_exit( LOG_ERR, "Error from setsockopt() in child( SOL_SOCKET, SO_RCVTIMEO ).");
    ...
    /* Verificamos que el modo de transferencia sea binario para seguir con la transferencia y
...
void wait_request(tftp_tl * listen) {
    ...
    if ( received == OPCODE_RRQ || received == OPCODE_WRQ )  {
        ...
        case 0:
            syslog(LOG_INFO, "Child created successfully with PID: %d", getpid());
            child( listen );
            _exit(EXIT_SUCCESS);
